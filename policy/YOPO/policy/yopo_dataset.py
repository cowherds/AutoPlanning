...
class YOPODataset(Dataset):
    def __init__(self, mode='train', val_ratio=0.1):
        ...
        self.feature_dim = int(cfg.get("pointcloud_feature_dim", 4))
        self.lidar_vertical_fov = cfg["lidar_vertical_fov"]
        self.lidar_sensing_horizon = cfg["lidar_sensing_horizon"]
        self.min_range = cfg.get("lidar_sensing_blind", 0.1)
        ...
        self.v_std = np.array([cfg["vx_std_unit"], cfg["vy_mean_unit"], cfg["vz_std_unit"]])
        ...
        self._load_lidar_dataset(data_dir, mode, val_ratio)

        print(f"=============== {mode.capitalize()} Data Summary ===============")
        print(f"{'Sensor':<12} | Mode: {'Point cloud':<8} | Shape: {self.num_points}x{self.feature_dim}")
    ...
    def _load_lidar_dataset(self, data_dir, mode, val_ratio):
        ...
        print("Loading", mode, "dataset (LiDAR / point cloud)")
        for data_idx in range(len(datafolders)):
            datafolder = datafolders[data_idx]
            lidar_file_names = [datafolder + "/" + filename
                                for filename in os.listdir(datafolder)
                                if os.path.splitext(filename)[1] == '.bin']
            lidar_file_names.sort(key=lambda x: int(os.path.basename(x).split('.')[0].split("_")[1]))
            ...
            file_names_train, file_names_val, positions_train, positions_val, quaternions_train, quaternions_val = train_test_split(
                lidar_file_names, positions, quaternions, test_size=val_ratio, random_state=0)
    ...
    def __getitem__(self, item):
        ...
        sensor_data = self._load_lidar_data(item)
        ...
        return sensor_data, self.positions[item], rot_wb, random_obs, self.map_idx[item]

    def _load_lidar_data(self, item):
        points = np.fromfile(self.img_list[item], dtype=np.float32).reshape(-1, 3)
        ...
        valid_depth = (depth > self.min_range) & (depth < self.lidar_sensing_horizon)
        ...
        sample_idx = self._sample_indices(points.shape[0], self.num_points)
        ...
        sensor_data = np.concatenate([points_norm, depth_norm], axis=1).astype(np.float32)
        return sensor_data

    @staticmethod
    def _sample_indices(src_num, target_num):
        if src_num >= target_num:
            return np.random.choice(src_num, target_num, replace=False)
        padding = np.random.choice(src_num, target_num - src_num, replace=True)
        return np.concatenate([np.arange(src_num), padding], axis=0)
    ...
    def plot_sample_distribution(self):
        ...
        pitch = np.degrees(np.arctan2(z, np.sqrt(x ** 2 + y ** 2)))
...
if __name__ == '__main__':
    ...
    print(f"\nBest num_workers = {best[0]}, avg time={best[1]:.3f}s")
