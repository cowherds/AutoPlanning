...
class YopoTrainer:
    ...
    def train_one_epoch(self, epoch: int, total_progress):
        ...
        for step, (sensor_points, pos, rot, obs_b, map_id) in enumerate(self.train_dataloader):  # obs: body frame
            if sensor_points.shape[0] != self.batch_size:  continue  # batch size == number of env

            self.optimizer.zero_grad()

            trajectory_loss, score_loss, smooth_cost, safety_cost, goal_cost, acc_cost = self.forward_and_compute_loss(
                sensor_points, pos, rot, obs_b, map_id
            )
    ...
    def eval_one_epoch(self, epoch: int):
        ...
        for step, (sensor_points, pos, rot, obs_b, map_id) in enumerate(self.val_dataloader):  # obs: body frame
            if sensor_points.shape[0] != self.batch_size:  continue  # batch size == num of env

            trajectory_loss, score_loss, _, _, _, _ = self.forward_and_compute_loss(sensor_points, pos, rot, obs_b, map_id)
    ...
    def forward_and_compute_loss(self, sensor_points, pos, rot, obs_b, map_id):
        sensor_points, pos, rot, obs_b, map_id = [x.to(self.device) for x in [sensor_points, pos, rot, obs_b, map_id]]
        ...
        endstate, score = self.policy.inference(sensor_points, obs_b)
