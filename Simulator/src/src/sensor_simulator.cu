    ...
    __global__ void lidarRaycastKernel(Vector3f* point_values, GridMap grid_map, LidarParams lidar_param, cudaMat::SE3<float> T_wc)
    {
        ...
        if (h < lidar_param.horizontal_num && v < lidar_param.vertical_lines)
        {
            ...
            while (1)
            {
                ...
                if (occupied == 1)
                {
                    // Return the hit point in LiDAR/body frame.
                    point_value = Vector3f(point_x, point_y, point_z);
                    break;
                }

                if (ray_length > lidar_param.max_lidar_dist){
                    // Keep max-range endpoints so RViz shows dense full-FOV scans.
                    point_value = Vector3f(
                        ray_direction.x * lidar_param.max_lidar_dist,
                        ray_direction.y * lidar_param.max_lidar_dist,
                        ray_direction.z * lidar_param.max_lidar_dist);
                    break;
                }
            }
